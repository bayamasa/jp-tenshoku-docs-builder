...
各会社に `job_type`（職種）を設定する。PDF上では会社名ヘッダーの下に「期間 | 職種・雇用形態」行として表示される。`employment_type` と全角スペース区切りで結合されるため、両方設定するのが望ましい。

例: `job_type: "バックエンドエンジニア"`, `employment_type: "正社員として勤務"`
...
- 各フィールドは `list[str]` 形式で記述する（`situation` / `task` は `str` でも可、自動変換）

#### STAR / description 記述ルール

YAML記述時に以下のルールが適用される。ユーザーにも案内すること。

| 記法 | 効果 |
|---|---|
| `。`（句点） | PDF上で句点の後に自動改行される |
| `- ""` | 空行スペーサー（段落間の区切り） |
...
副業 (`side_experience`) の `description` にも同じルールが適用される。

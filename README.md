...
PDF上で各会社は以下の構成で表示されます:

1. **会社名**（グレー背景ヘッダー行）
2. **期間 | 職種・雇用形態**（`period` / `job_type` + `employment_type` を全角スペース区切りで結合）
3. プロジェクトテーブル（期間 / 業務内容 / 開発環境 / 役割）

`job_type` フィールドで職種（例: "バックエンドエンジニア"）を指定できます。

#### YAML記述ルール

STAR法形式のプロジェクト内容（`situation` / `task` / `action` / `result`）と副業の `description` に共通するルールです。
...
`situation` / `task` は `str` でも `list[str]` でも記述可能です（`str` は自動で `list[str]` に変換されます）。
